...
**콜드 로드에서는 발화하지 않는다 — 새로고침(`isRefresh === true`)일 때만 보고한다.** 콜드 로드는 이미 `HistorySkeleton`이 로딩을 말하고 있고, 사용자가 누르지도 않은 버튼이 도는 것은 "누른 버튼이 반응한다"는 §2 결정과 어긋난다. 즉 이 콜백은 자식의 `loading` 상태를 그대로 중계하는 것이 아니라, **사용자가 시작시킨 작업의 진행 여부**만 보고한다.

`Header`에 `refreshing: boolean` prop 추가.
...
`App.test.tsx`가 `screen.getByText("Refresh")`로 새로고침 버튼을 찾는다 (`App.test.tsx:58`, `:72`, `:106`). 버튼에 아이콘을 넣으면 `textContent`가 `"↻Refresh"`가 되어 이 쿼리가 깨진다.

→ 라벨을 별도 엘리먼트로 감싼다: `<button><Spinner/><span>{t("app.refresh")}</span></button>`. `getByText("Refresh")`가 내부 span에 매치되고, 클릭은 버튼으로 버블링되어 기존 테스트가 그대로 통과한다.
