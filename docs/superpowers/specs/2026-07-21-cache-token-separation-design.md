...
`MonthlyDetail`은 로그 원본 값만 담는다. 같은 이름의 `MonthlySummary` 필드가 정규화된 값을
담으므로, Detail의 원시 버킷 5개에 `raw_` 접두어를 붙여 둘을 이름으로 구분한다. 접두어를
일부에만 붙이면 "왜 input만 raw_인가"라는 질문이 남으므로 전부 붙인다.
...
pub direct_tokens: u64,   // 신규 — 파생값이므로 접두어 없음
```

`MonthlySummary`에 필드 5개 추가:
...
`MonthlySummary`의 네 버킷은 **표시용으로 정규화된 값**이다 (아래 표 참조). 원시 필드와 이름이
같지만 Codex에서는 다른 원본에서 오거나 값이 조정된다 — 필드 doc comment에 명시한다.
`MonthlyDetail`은 감사 가능성을 위해 원시 버킷을 그대로 유지하고 `direct_tokens`만 더한다.

### 2. 집계 규칙 (`src-tauri/src/history.rs`)

정규화는 `display_buckets()` 헬퍼 한 곳에서만 일어나고, detail(원시 레코드에서)과
summary(detail에서) 양쪽이 이 함수를 호출한다. 규칙이 두 벌 생기지 않게 하는 것이 요점이다.

| 표시 버킷 | Claude | Codex |
|---|---|---|
| `input_tokens` | `input` | `input.saturating_sub(cached_input)` |
| `output_tokens` | `output` | `output` |
| `cache_read_tokens` | `cache_read` | `cached_input.min(input)` |
| `cache_write_tokens` | `cache_write` | `0` (Codex는 캐시 쓰기 개념 없음) |
| `direct_tokens` (파생) | `input + output` | `input − cached_input + output` |
| `total_tokens` | `input + output + cache_write + cache_read` (변경 없음) | `input + output` (변경 없음) |

이 정의에서 **네 버킷의 합이 항상 `total_tokens`와 일치한다.** Claude는
`input + output + cacheR + cacheW = total`, Codex는 `(input − cached) + output + cached + 0 =
input + output = total`. 확장 행이 산술적으로 앞뒤가 맞아야 사용자가 숫자를 신뢰한다.

Codex 쪽의 `saturating_sub`와 `min`은 **짝으로 동작해야 한다.** `cached_input > input`인 이상치
로그에서 `input` 쪽만 0으로 막고 `cache_read`에 원본을 그대로 넣으면 버킷 합이 총합을 넘어선다
(`input=100, output=40, cached=900` → 총합 140인데 버킷 합 940). 두 값 모두 `input`을 상한으로
잡아야 `input' + cache_read' == input`이 유지되어 불변식이 성립한다. 정상 데이터
(`cached ≤ input`)에서는 `min`이 아무것도 바꾸지 않는다.

u64 언더플로는 디버그 빌드에서 패닉, 릴리스에서 거대한 쓰레기 값이 되므로 방어가 필요하다.

요약 합산 루프(`sums`)는 튜플 대신 작은 누적 구조체로 바꾼다. 누적할 값이 6개가 되면
`e.0`, `e.3` 같은 인덱스 접근이 읽히지 않는다.

### 3. 요약 카드 (`src/components/UsageHistoryView.tsx`)

```
┌─ Claude ─────────────────┐
│ 9,335,674 토큰           │  ← direct_tokens, 기존 .history-card-tokens 스타일
│ 캐시 포함 2,147,671,485  │  ← total_tokens, 작고 흐리게 (신규 .history-card-cached)
│ $123.45                  │  ← 변경 없음
└──────────────────────────┘
```

`formatTokens`는 `Intl.NumberFormat("en-US")`로 전체 자릿수를 콤마 구분해 출력한다. "2.15B"
같은 축약은 도입하지 않는다 — 축약은 별개의 표시 결정이고 위젯·xlsx와 표기가 갈리게 된다.
...
| 파일 | 변경 |
|---|---|
| `src-tauri/src/model.rs` | `MonthlyDetail` +1, `MonthlySummary` +5 필드 |
...
| `src/locales/{ko,en}.json` | 신규 키 8개, `colTokens` 문구 변경 |
