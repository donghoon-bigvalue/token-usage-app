...
구버전을 더 이상 쓰게 두면 안 되는 릴리스는 **강제 업데이트**로 낼 수 있습니다.
`CHANGELOG.md`의 해당 버전 섹션에 마커 한 줄을 넣으면 됩니다 — 릴리스 워크플로가
이 섹션을 그대로 릴리스 노트와 `latest.json`에 싣기 때문에 별도 설정이 필요 없습니다.
...
마커는 `<!-- force-update -->`(릴리스 페이지에 보이지 않음) 또는 `[force-update]`
둘 다 인식하며, 대소문자와 `_`·공백 구분자를 가리지 않습니다.

이 릴리스를 만난 앱은 **닫을 수 없는 팝업**을 띄웁니다 — `[다음에 하기]`가 사라지고,
`[다운로드 페이지 열기]`(릴리스 페이지)와 `[자동 업데이트]`만 남습니다. 이전에 같은
버전을 "다음에 하기"로 넘겼더라도 다시 표시됩니다.

> 시작 시 확인은 하루 1회 스로틀을 그대로 따르므로, 강제 릴리스도 최대 24시간 뒤에
> 발견될 수 있습니다. 또한 업데이트 확인에 성공해야 마커를 읽으므로 오프라인 상태에서는
> 팝업이 뜨지 않습니다.
