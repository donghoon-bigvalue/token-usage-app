...
강제 업데이트 팝업은 config의 `minimumVersion`을 올려야 재현되는데, 그건 전 사용자에게
나가는 조작이라 확인용으로 쓸 수 없습니다. 대신 개발 빌드에서 **설정 → 업데이트** 아래에
미리보기 버튼이 나옵니다.

```bash
npm run tauri dev     # 또는 npm run dev (프런트엔드만)
```

`일반` / `강제` / `강제 · 받을 것 없음` / `강제 · 정책 문구` 네 가지 상태를 띄워 볼 수 있고,
`자동 업데이트`를 누르면 다운로드 진행률까지 흉내 냅니다. 강제 팝업에는 닫기 버튼이 없으니
**Esc** 또는 우상단 `미리보기 종료`로 빠져나오면 됩니다.

이 UI는 `import.meta.env.DEV` 뒤에 있어 프로덕션 번들에는 코드·문구·스타일 어느 것도
포함되지 않습니다 (`src/components/UpdatePreview.tsx`).
